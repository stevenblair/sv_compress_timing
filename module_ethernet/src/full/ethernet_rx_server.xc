...
//  debug_printf("%x\n", result);
  ...
  if (result) {
    // TODO base this decision on src_port and match?
    ...
    for (i = 0; i < n; i += 1) {
      ...
      if (match && src_port == 1) {
//        debug_printf("match, link %d, source port %d\n", n, src_port);
...
//      debug_printf("forwarding from source port %d\n", src_port);
      tcount++;
