...
#pragma xta label "mii_tx_main_loop"
        ...
        if (!buf || (mii_packet_get_stage(buf) == 0)) {
          for (unsigned int i=0; i<NUM_ETHERNET_PORTS; ++i) {
            ...
            if (!buf)
              continue;
//            debug_printf("MII master, forwarding from source port %d to %d\n", ifnum, i);
...
//        int recv_time = mii_packet_get_timestamp(buf);
//        debug_printf("port %d, diff %d us\n", ifnum, (ifg_time - recv_time) / 100);
